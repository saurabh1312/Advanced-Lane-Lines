...
for file_name in images:
    ...
    output_name = './output_images/undistorted_images/' + os.path.basename(file_name)
    ...
    processedImage[((gradx == 1) & (grady == 1) | (color_binary == 1))] = 255

    # Save thresholded binary images
    output_name = './output_images/binary_images/' + os.path.basename(file_name)
    cv2.imwrite(output_name, processedImage)
    ...
    # Save warped images
    output_name = './output_images/warped_images/' + os.path.basename(file_name)
    ...
    # Set up the slider
    curve_centers = Slider(width=window_width, height=window_height, pixel_margin=25, ym=10/720, xm=4/384, smooth=15)
    ...
    slided = cv2.addWeighted(warp, 1, template, 0.5, 0.0)  # Overlay window results to original image

    # Save slider images
    output_name = './output_images/sliding_window/' + os.path.basename(file_name)
    cv2.imwrite(output_name, slided)
    ...
    # Save polyfit images
    output_name = './output_images/polyfit_images/' + os.path.basename(file_name)
    cv2.imwrite(output_name, road)
    ...
    original_overlaid = cv2.addWeighted(original_overlaid_bkg, 1.0, original_overlaid, 1.0, 0.0)
    ...
    cv2.putText(original_overlaid, 'Radius of Curvature = ' + str(round(curverad, 3)) + '(m)', (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    cv2.putText(original_overlaid, 'Car is ' + str(abs(round(center_diff, 3))) + 'm ' + side_pos + ' of center', (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

    # Save lane lines on original images
    output_name = './output_images/final_output/' + os.path.basename(file_name)
    cv2.imwrite(output_name, original_overlaid)
